...
all_genres = sorted({genre for sublist in movies["genres_list"] for genre in sublist})
selected_genres = st.sidebar.multiselect("Genres:", all_genres, default=["Comedy", "Action"])
df_filtered = df[df["genres_list"].apply(lambda x: any(g in x for g in selected_genres))]
...
fig1 = px.bar(genre_counts.sort_values("rating_count", ascending=False).head(10),
              x="genres_list", y="rating_count", color="avg_rating",
              title="Most Rated Genres", color_continuous_scale="Turbo")
st.plotly_chart(fig1)

# Plot 2: Rating Distribution
fig2 = px.histogram(df_filtered, x="rating", nbins=20, title="Ratings Distribution")
st.plotly_chart(fig2)

# Plot 3: Genre over Time
genre_year_summary = (
    df.explode("genres_list")
    .groupby(["genres_list", "year"])
    .agg(avg_rating=("rating", "mean"), rating_count=("rating", "count"))
...
fig3 = px.bar(
    genre_year_summary[genre_year_summary["genre"].isin(selected_genres)],
    x="genre", y="rating_count", color="avg_rating", animation_frame="year",
    title="Genre Popularity Over Time", color_continuous_scale="Turbo"
)
st.plotly_chart(fig3)

st.markdown("---")
st.markdown("This dataset is perfect for ML applications in online retail: genre trends, ratings, user preferences, and scale.")
